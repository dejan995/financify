...
import { DollarSign, Globe, RefreshCw, ShoppingBag, Package, TrendingUp as CashFlowIcon, PiggyBank, Wallet, Server, CreditCard, Loader2 } from 'lucide-react';
import { currencies } from '@/lib/currencies';

const CurrencySettingsDisplay = ({ settings, setSettings, handleRefreshRates, refreshingRates }) => {
  ...
  const sectionCurrencySettings = [
    ...
    { id: 'budgetCurrency', label: 'Budgets', icon: Wallet },
    { id: 'servicesCurrency', label: 'Services', icon: Server },
    { id: 'creditCardsCurrency', label: 'Credit Cards', icon: CreditCard },
  ];
      ...
      <Card className="glassmorphism border-indigo-500/50">
        ...
        <CardContent className="space-y-6">
          ...
          {settings.enableCurrencyConversion && (
            <>
              <div className="flex items-center justify-between p-3 rounded-lg bg-slate-900/30">
                <Label htmlFor="original-switch" className="font-medium text-slate-300">Show Original Currency Value</Label>
...
const CurrencySettingsContainer = () => {
    ...
    const [settings, setSettings] = useState({
        ...
        expensesCurrency: context.expensesCurrency,
        productsCurrency: context.productsCurrency,
        cashFlowCurrency: context.cashFlowCurrency,
        savingsCurrency: context.savingsCurrency,
        budgetCurrency: context.budgetCurrency,
        servicesCurrency: context.servicesCurrency,
        creditCardsCurrency: context.creditCardsCurrency,
        enableCurrencyConversion: context.enableCurrencyConversion,
        showOriginalCurrency: context.showOriginalCurrency,
    });
    ...
    const handleSave = async () => {
      setIsSaving(true);
      await context.updateUserPreferences(settings);
      setIsSaving(false);
    };
...
export default CurrencySettingsContainer;